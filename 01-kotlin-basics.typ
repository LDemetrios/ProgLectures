...
Все эти типы заимствованы из JVM. Собственно, из неё же заимствованы ещё и типы #KtByte (8-битное
целое), #KtShort (16-битное целое) и #KtFloat (single-precision вещественное),
но их используют крайне редко (поскольку всё меньше остаётся даже 32-битных
процессоров).
...
stdout (стандартный поток вывода). А #KtUnit --- это тип, который возвращается,
если не возвращается ничего. Есть в Котлине два таких интересных типа: #KtUnit --- когда
единственное, что нам нужно знать --- это завершилась функция или нет. Если
завершилась, то возвращается #KtUnit --- единственное значение типа #KtUnit. Ну,
содержательного с ним ничего нельзя сделать --- только сказать, что он есть.
Второй --- #KtNothing --- означает, что функция никогда ничего не вернёт, потому
что не завершится успешно. Если у предыдущего типа существует единственное
значение, то у типа #KtNothing нет ни одного значения.
...
А нет. Цикл не является выражением, он не возвращает даже #KtUnit. Как, впрочем,
...
Если вы можете использовать #kt-fmt(`val`), используйте #kt-fmt(`val`), а не #kt-fmt(`var`).
...
скобках могут быть ещё аргументы, например, #kt-fmt(`566.toString(3)`), чтобы
преобразовать в троичную систему счисления. Все инфиксные функции можно вызывать
также и в условно-постфиксной записи: #kt-fmt(`0.until(5)`), но не наоборот.
...
Что бы всё это значило? #kt-fmt(`fun`) собственно объявляет функцию. `a` и `b`
--- это имена параметров, после них написаны их типы (#KtInt), а после двоеточия
после всего объявления тип того, что возвращает функция. В нашем случае функция
принимает два аргумента типа #KtInt и возвращает тоже #KtInt. Теперь собственно,
...
Так, кроме того, мы ввели ключевое слово return. Кроме него существуют ещё такие интересные штуки как break, continue и throw, которые делают понятно что. Но их объединяет следующая черта: после них точно будет выполняться уже не эта строка. В некотором смысле, это выражение никогда ничего не возвращает. О, где-то такое уже было. На самом деле на уровне языка оно возвращает тип #KtNothing. Например, если мы напишем #kt-fmt(`val x = return`), то понятно, что до присвоения значения постоянной `x` дело так и не дойдёт.

Для понимания следующего сначала рассмотрим обобщающие типы. Так, если мы пишем #kt-fmt(`if(condition) 1 else 2`), то понятно, какой тип выведет компилятор --- #KtInt. А если типы возвращаемых данных разные? Ошибка? 
#kt-ea(`
if(1 < 2) 3 else "4"
`)
#kt-res(`3`, Comparable(KtStar))
А нет. Что-то мы всё же про это знаем, и числа, и строки можно сравнивать. А значит, мы на выходе получили что-то _сравниваемое_ (#Comparable(KtStar)). Что означает звёздочка в треугольных скобках --- мы потом разберём, сейчас это несущественно. А бывают типы данных, которые нельзя сравнивать? Бывают, например #KtUnit. Ну, его сравнивать довольно бесполезно --- он всегда окажется равен самому себе, поскольку единственное, что существует типа #KtUnit --- это #KtUnit сам по себе. #kt-ea(`
if(1 < 2) 3 else Unit
`)
#kt-res(`3`, Any)
...
Так вот, всё, что не null --- является Any, и может быть приведён к типу Any. А тип Nothing --- наоборот, может быть приведён *_к_* любому типу. Потому что на самом деле никогда не понадобится приводить, потому что объектов типа Nothing не существует.